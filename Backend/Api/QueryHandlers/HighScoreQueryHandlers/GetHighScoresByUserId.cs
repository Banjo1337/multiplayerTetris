...
public class GetHighScoresByUserId
{
    public record Query(Guid userId) : IRequest<IEnumerable<HighScoreResponseDto>>;
    ...
    public class Handler : IRequestHandler<Query, IEnumerable<HighScoreResponseDto>>
    {
        ...
        {
            return _context.HighScores.Where(h => h.UserId == request.userId).Select(h => new HighScoreResponseDto()
            {
                Id = h.Id,
                UserId = h.UserId,
                Score = h.Score,
